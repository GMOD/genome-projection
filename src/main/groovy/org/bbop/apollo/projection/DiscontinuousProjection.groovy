...
//            println "nextMin ${coordinate.min} -> higher key -> ${nextMin} < ${max} "
    ...
    Coordinate addInterval(int min, int max, Integer padding = 0) {
        min -= padding ?: 0
        max += padding ?: 0
        min = min < 0 ? 0 : min
//        println "${min},${max}"
...
//            floorMinCoord == floorMaxCoord
        ...
        if (floorMinCoord != null && floorMaxCoord == null && ceilMinCoord == null && ceilMaxCoord != null) {
            ...
            return replaceCoordinate(floorMinCoord, Math.min(min, floorMinCoord.min).intValue(), Math.max(max, ceilMaxCoord.max).intValue())
        }
        // overlapping without?
        if (floorMinCoord == null && floorMaxCoord != null && ceilMinCoord != null && ceilMaxCoord == null) {
            assert floorMaxCoord == ceilMinCoord
            return replaceCoordinate(floorMaxCoord, Math.min(min, floorMaxCoord.min).intValue(), Math.max(max, ceilMinCoord.max).intValue())
        }
